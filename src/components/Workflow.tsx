...
const steps = [
  {
    id: 1,
    title: 'Connect',
    desc: 'Mobile (Sequence AA) or Web (EOA)',
    icon: <Wallet size={24} />,
  },
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            {steps.map((step, index) => (
              ...
              >
                ...
                {/* Mobile Connector */}
                {index < steps.length - 1 && <div className="md:hidden w-1 h-8 bg-white/10 my-4" />}
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-16 text-center">
          <div className="inline-block bg-white/5 border border-white/10 rounded-lg px-6 py-4">
            ...
            <p className="text-sm text-gray-300">
              Tasks reset daily at <span className="text-white font-bold">00:00 (UTC)</span>
            </p>
          </div>
        </div>
      </div>
