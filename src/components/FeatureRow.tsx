...
export const FeatureRow: React.FC<FeatureRowProps> = ({ feature, index }) => {
  ...
  return (
    <section ref={ref} className="min-h-[60vh] flex items-center justify-center py-20 relative">
      <div
        className={`w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${isEven ? '' : 'lg:grid-flow-dense'}`}
      >
        {/* Text Content */}
        <motion.div
        ...
        >
          ...
          <div className="relative w-full h-full pt-8">{feature.visual}</div>

          {/* Corner Accents */}
          <div
            className={`absolute bottom-0 right-0 w-16 h-16 bg-gradient-to-tl from-${feature.color.replace('text-', '')}/10 to-transparent`}
          />
        </motion.div>
      </div>
    </section>
