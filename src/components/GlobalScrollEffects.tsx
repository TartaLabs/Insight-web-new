...
export const GlobalScrollEffects: React.FC = () => {
  ...
  // Pre-create transforms for warp lines to avoid hooks in callbacks
  const warpTransforms = useMemo(() => {
    return [0, 1, 2, 3, 4, 5].map((i) => 0.05 + i * 0.03);
  }, []);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-30 overflow-hidden">
      ...
      <div className="absolute inset-0 flex justify-between px-[10%] opacity-20">
        {warpTransforms.map((multiplier, i) => (
          <motion.div
            key={`warp-${i}`}
            ...
            style={{
              ...
              y: useTransform(scrollY, (v) => v * multiplier),
            }}
          />
        ))}
      </div>
      ...
      <div className="absolute right-0 top-0 h-full w-12 flex flex-col justify-center items-end pr-2 gap-1 mix-blend-screen">
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-2">
          <span className="text-[10px] font-mono text-white bg-tech-blue/20 px-1 rounded">
            Y-AXIS
          </span>
