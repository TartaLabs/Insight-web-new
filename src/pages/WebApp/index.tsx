...
// Helper function to validate invite codes
const createValidateInviteCode =
  (inviteUsage: Record<string, { owner: string; used: number }>) => (code: string) => {
...
export const WebApp: React.FC<WebAppProps> = ({ onExit }) => {
  ...
  const [inviteCodeInfo, setInviteCodeInfo] = useState<{
    code: string;
    invitedBy: string;
    locked: boolean;
    persisted: boolean;
  ...
  const [inviteUsage, setInviteUsage] =
    useState<Record<string, { owner: string; used: number }>>(INITIAL_INVITE_USAGE);
  ...
  const validateInviteCode = React.useCallback(
    (code: string) => createValidateInviteCode(inviteUsage)(code),
    [inviteUsage],
  );
  ...
  }, [inviteUsage, validateInviteCode]);
  ...
  const handleLogin = (nickname: string) => {
    if (inviteCodeInfo.code) {
      const bindResult = bindInviteCode(inviteCodeInfo.code, {
        lock: true,
        persist: !inviteCodeInfo.persisted,
      });
  ...
  if (!isLoggedIn) {
    ...
    return (
      <Login
        ...
        onPersistInvite={(code) => bindInviteCode(code, { lock: true, persist: true })}
      />
    );
  }
  ...
  return (
    <div className="relative min-h-screen bg-[#020205] text-white">
      ...
      {showUpgrade && (
        <UpgradeModal user={user} onClose={() => setShowUpgrade(false)} onUpgrade={handleUpgrade} />
      )}
