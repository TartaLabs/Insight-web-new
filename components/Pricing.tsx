    ...
    <div className="w-full py-24 container mx-auto px-6">
      <div className="flex flex-col md:flex-row justify-between items-end mb-16">
        <div>
           ...
           <p className="text-gray-400 text-lg">Maximize your contribution efficiency and $mEMO earnings.</p>
        </div>
      </div>
          ...
          >
            ...
            <div className="mb-6">
               ...
               <div className="flex items-baseline gap-1">
                  <span className={`text-3xl font-bold ${plan.recommended ? 'text-neon-purple' : 'text-white'}`}>{plan.price}</span>
            ...
            <div className="grid grid-cols-2 gap-2 mb-6">
               <div className="bg-black/40 p-2 rounded border border-white/5">
                  ...
                  <div className="text-lg font-bold text-white">{plan.dailyLimit}x <span className="text-[10px] font-normal text-gray-600">/emo</span></div>
               </div>
               <div className="bg-black/40 p-2 rounded border border-white/5">
                  <div className="text-[10px] text-gray-500 uppercase">Earn</div>
                  <div className="text-lg font-bold text-neon-blue">{plan.rewardPerTask} <span className="text-[10px] font-normal text-gray-600">$mEMO</span></div>
               </div>
            ...
            <ul className="space-y-3 mb-8 flex-1">
              {plan.features.map((feat, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs text-gray-300">
                  <Check size={14} className={plan.recommended ? 'text-neon-purple' : 'text-gray-500'} />
                  {feat}
                </li>
              ))}
            </ul>

            <button className={`w-full py-3 text-xs font-bold tracking-widest rounded transition-all ${
              plan.recommended 
              ? 'bg-neon-purple text-white hover:bg-white hover:text-black' 
              : plan.id === 'basic' ? 'bg-white/10 text-white hover:bg-white/20' : 'border border-white/20 text-white hover:border-white'
            }`}>
