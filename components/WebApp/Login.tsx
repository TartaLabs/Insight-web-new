...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess, onBack, inviteCode, inviteLocked, invitedBy, existingNicknames, onInviteChange, onValidateInvite, onPersistInvite }) => {
  ...
  const handleConnect = () => {
    ...
    setTimeout(() => {
      ...
      if (stored) {
        try {
          const data = JSON.parse(stored) as { nickname?: string; inviteCode?: string; invitedBy?: string };
          if (data.nickname) {
  ...
  const handleEnterApp = () => {
    ...
    if (walletAddress) {
      localStorage.setItem(`insight_profile_${walletAddress}`, JSON.stringify({
        nickname: nick,
             ...
             <div className="space-y-6">
                <div>
                   ...
                   <div className="relative">
                    ...
                    />
                     <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-gray-500 font-mono">{nickname.length}/15</span>
                   </div>
                   <div className="text-[11px] text-gray-500 mt-1">Use 1-15 letters or numbers. Must be unique.</div>
                ...
                <div>
                   <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Invitation Code</label>
                   <input
                      type="text"
                      value={inviteInput}
                      onChange={(e) => {
                   ...
                   <div className="flex justify-between items-center mt-2 text-[11px] text-gray-500">
                      <span>{inviteLocked ? 'Invite code locked for this session' : 'Optional'}{inviteOwner && ` · Invited by ${inviteOwner}`}</span>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.2em] text-gray-500">Legal Notice</p>
