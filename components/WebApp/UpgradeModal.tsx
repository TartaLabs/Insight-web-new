...
export const UpgradeModal: React.FC<UpgradeModalProps> = ({ user, onClose, onUpgrade }) => {
  ...
  const [step, setStep] = useState<'select' | 'approve' | 'confirm'>('select');
  ...
  const handleProcess = () => {
    ...
    setTimeout(() => {
        setStep('approve'); // 1.5s approving USDT
        setTimeout(() => {
             setStep('confirm'); // 1.5s Confirming Transaction
             setTimeout(() => {
        ...
        <div className="p-8">
           <div className="flex justify-between items-start mb-2">
               ...
               <div className="text-right">
                   ...
                   <div className={`font-mono font-bold ${canAfford ? 'text-green-500' : 'text-red-500'}`}>
                       {user.balanceUSDT.toFixed(2)} USDT
                   </div>
               </div>
           </div>
           <p className="text-gray-400 mb-8">Pay with <span className="text-green-400 font-bold">USDT</span> on Mantle Network</p>
           ...
           {step === 'select' ? (
              ...
              <div className="flex flex-col items-center justify-center py-12">
                 <div className="w-16 h-16 rounded-full border-4 border-white/10 border-t-tech-blue animate-spin mb-6" />
                 <h3 className="text-xl font-bold animate-pulse">
                    {step === 'approve' ? 'Approving USDT...' : 'Confirming Transaction...'}
                 </h3>
                 <p className="text-gray-500 mt-2 font-mono text-sm">Please confirm in your wallet (~0.01 MNT Gas)</p>
              </div>
           )}
        </div>
        ...
        {step === 'select' && (
           <div className="p-6 border-t border-white/10 flex justify-between items-center bg-black/20">
              <div className="text-sm text-gray-400">
                 Selected: <span className="text-white font-bold">{selectedPlanDetails?.name || 'None'}</span>
              </div>
              ...
              <div className="flex items-center gap-4">
                  ...
                  <button 
                     ...
                     className={`px-8 py-3 rounded font-bold flex items-center gap-2 ${
                        !selectedPlan || !canAfford || !hasGas
