        ...
        <div className="p-8">
           <div className="flex justify-between items-start gap-4 mb-3">
               <div>
                   ...
                   <p className="text-gray-400 mt-2">Pay with <span className="text-green-400 font-bold">USDT</span> on Mantle Network</p>
                   <div className="text-[11px] text-gray-500 font-mono mt-1">Wallet: {user.walletAddress || 'Connected'}</div>
               </div>
              ...
              <div className="flex flex-col gap-6 py-6">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                      ...
                      const done = (step === 'pay' && s.id === 'approve') || (step === 'done' && s.id !== 'select');
                      return (
                        <div key={s.id} className={`p-4 rounded border text-center ${active ? 'border-tech-blue bg-tech-blue/10 text-white' : done ? 'border-green-500/40 bg-green-500/10 text-green-200' : 'border-white/10 text-gray-400'}`}>
                           <div className="text-sm font-bold">{s.label}</div>
                           <div className="text-[11px] text-gray-500 mt-1">{active ? 'Waiting for wallet...' : done ? 'Done' : 'Pending'}</div>
                        </div>
                      );
                    })}
                 </div>
