...
import { UserProfile, TaskCounts, EmotionType, LeaderboardEntry, Transaction, TaskRecord, Invitee, SubscriptionRecord } from '../../types';
import { Crown, Clock, Share2, Zap, Coins, Copy, Trash2, Edit2, CheckCircle2, XCircle, Timer, Wallet, ExternalLink, RefreshCw, Filter, Users, Calendar, Shield, Activity, X, CreditCard, Flame } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
    user, tasks, taskCounts, dailyLimit, rewardPerTask, history, leaderboard, invitees, ownInviteCode, inviteLink, subscriptions, inviteCodeInfo,
    onRenameNickname, onStartTask, onResumeTask, onDeleteTask, onUpgradeClick, onClaimAll, onClaimBonus, onRetryClaim, onClaimInvitationRewards, onApplyInviteCode
}) => {
  const formatAddress = (addr: string) => addr ? `${addr.slice(0, 6)}...${addr.slice(-4)}` : '';
  const [activeTab, setActiveTab] = useState<'contributions' | 'memo_history' | 'leaderboard' | 'invitation' | 'subscriptions'>('contributions');
  ...
  const computedLeaderboard = useMemo(() => {
    ...
    const hasSelf = list.some(e => e.nickname.toLowerCase() === (user.nickname || '').toLowerCase() || e.address === user.walletAddress);
    ...
    list = list.sort((a, b) => b.totalEarned - a.totalEarned).map((e, idx) => ({ ...e, rank: idx + 1 }));
    return list;
  }, [leaderboard, user.balanceMEMO, user.nickname, user.walletAddress]);

  const selfEntry = computedLeaderboard.find(e => e.nickname.toLowerCase() === (user.nickname || '').toLowerCase() || e.address === user.walletAddress);
  ...
  const inviteLinkDisplay = inviteLink || (typeof window !== 'undefined' 
           ...
           <div className="flex items-center gap-5 w-full">
              <div className="relative w-16 h-16 flex-shrink-0">
                 ...
                 <div className="absolute inset-2 bg-gradient-to-b from-blue-900 to-black rounded-full flex items-center justify-center border border-white/10">
                    <span className="text-xl font-bold text-white">{user.nickname ? user.nickname.charAt(0).toUpperCase() : '?'}</span>
                 </div>
                 {user.isPro && (
              ...
              <div className="flex-1">
                 <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => { setNicknameModalOpen(true); setNicknameError(''); }}
                 ...
                 <div className="flex items-center gap-2 mt-2">
                   <code className="text-xs text-gray-500 bg-black/50 px-2 py-1 rounded border border-white/5">{formatAddress(user.walletAddress)}</code>
                    <button onClick={() => copyToClipboard(user.walletAddress, 'Address copied')} className="text-gray-500 hover:text-tech-blue transition-colors">
                      <Copy size={12} />
                    </button>
                 </div>
                      ...
                      <div className="space-y-2">
                          <div className="grid grid-cols-12 text-[10px] text-gray-500 uppercase tracking-wider pb-2 border-b border-white/10 px-4">
                              ...
                              {submittedTasks.length === 0 ? (
                                  ...
                                  submittedTasks.slice((uploadPage-1)*UPLOAD_PAGE_SIZE, (uploadPage-1)*UPLOAD_PAGE_SIZE + UPLOAD_PAGE_SIZE).map(task => (
                              ...
                              {submittedTasks.length > 0 && (
                                <div className="flex justify-center items-center gap-3 pt-2">
                                  ...
                                  <div className="text-[11px] text-gray-400">
                                    Page <span className="text-white">{uploadPage}</span> / <span className="text-white">{uploadTotalPages}</span>
                                  </div>
                                  <button
                  ...
                  <div>
                      ...
                      <div className="space-y-2 font-mono">
                          {filteredLedger.length === 0 ? (
                            ...
                            <>
                              {filteredLedger.slice((ledgerPage-1)*LEDGER_PAGE_SIZE, (ledgerPage-1)*LEDGER_PAGE_SIZE + LEDGER_PAGE_SIZE).map(tx => (
                              ...
                              <div className="flex justify-center items-center gap-3 pt-2">
                                ...
                                <div className="text-[11px] text-gray-400">
                                  Page <span className="text-white">{ledgerPage}</span> / <span className="text-white">{ledgerTotalPages}</span>
                                </div>
                                <button
                ...
                <div className="space-y-3">
                  ...
                  {subscriptions.length === 0 ? (
                    ...
                    <>
                      ...
                      {subscriptions.slice((subPage-1)*subscriptionsPageSize, (subPage-1)*subscriptionsPageSize + subscriptionsPageSize).map(sub => (
                        <div key={sub.id} className="grid grid-cols-12 items-center text-xs px-2 py-3 bg-white/5 border border-white/5 hover:bg-white/10 transition-colors">
                          <div className="col-span-3 font-mono text-gray-400">{new Date(sub.createdAt).toLocaleString()}</div>
                          <div className="col-span-3 text-white font-bold truncate">{sub.planName}</div>
                          <div className="col-span-2 text-right font-mono text-white">{sub.amountUSDT.toFixed(2)} USDT</div>
                          <div className="col-span-2 text-center text-gray-400">{sub.chain}</div>
                          <div className="col-span-2 text-right">
                            {sub.status === 'SUCCESS' && <span className="text-green-500 font-bold">SUCCESS</span>}
                            {sub.status === 'PENDING' && <span className="text-yellow-500 font-bold">PENDING</span>}
                            {sub.status === 'FAILED' && <span className="text-red-500 font-bold">FAILED</span>}
                            {sub.txHash && <div className="text-[9px] text-gray-500 truncate">Tx: {sub.txHash}</div>}
                          </div>
                        </div>
                      ))}
                      <div className="flex justify-center items-center gap-3 pt-2">
                        ...
                        <div className="text-[11px] text-gray-400">
                          Page <span className="text-white">{subPage}</span> / <span className="text-white">{Math.max(1, Math.ceil(subscriptions.length / subscriptionsPageSize))}</span>
                        </div>
                        <button
                          onClick={() => setSubPage(p => Math.min(Math.max(1, Math.ceil(subscriptions.length / subscriptionsPageSize)), p+1))}
                  ...
                  <div className="space-y-4 pt-8"> {/* Added pt-8 to prevent overflow */}
                      ...
                      <div className="grid grid-cols-3 gap-6 mb-8 items-end">
                          {[1, 0, 2].map((idx) => { // Render 2nd, 1st, 3rd order
                              ...
                              const isFirst = idx === 0;
                              const ringColor = isFirst ? 'ring-yellow-400' : idx === 1 ? 'ring-gray-300' : 'ring-amber-500/80';
                              const glow = isFirst ? 'shadow-[0_0_25px_rgba(234,179,8,0.5)]' : idx === 1 ? 'shadow-[0_0_20px_rgba(203,213,225,0.35)]' : 'shadow-[0_0_20px_rgba(255,191,71,0.35)]';
                              return (
                                  <div key={idx} className={`relative flex flex-col items-center ${isFirst ? 'scale-110 z-10' : 'scale-95 opacity-90'}`}>
                                      <div className="relative">
                                          <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center bg-black ring-4 ${isFirst ? 'border-yellow-500' : 'border-white/20'} ${ringColor} ${glow}`}>
                                          ...
                                          <div className="absolute -top-6 left-1/2 -translate-x-1/2 flex items-center justify-center">
                                            <Crown className={`${isFirst ? 'text-yellow-400' : idx === 1 ? 'text-gray-200' : 'text-amber-400'} drop-shadow-lg`} size={24} />
                                          </div>
                                      </div>
                                      <div className="mt-4 text-center">
                                          ...
                                          <div className="text-sm font-bold text-tech-blue mt-1">{entry.totalEarned.toLocaleString()} <span className="text-[10px] text-gray-400">$mEMO</span></div>
                                      </div>
                                  </div>
                              );
                          })}
                      ...
                      <div className="space-y-2">
                          <div className="flex justify-end mb-3 pr-1">
                            <div className="px-4 py-3 rounded border border-white/10 bg-white/5 text-xs font-mono text-gray-300 flex items-center gap-3 shadow-[0_0_12px_rgba(0,243,255,0.08)]">
                              ...
                              <div className="text-white font-bold text-sm">{user.balanceMEMO.toLocaleString()} <span className="text-[10px] text-gray-500">$mEMO</span></div>
                            </div>
                          </div>
                          {otherEntries.slice((page - 1) * PAGE_SIZE, (page - 1) * PAGE_SIZE + PAGE_SIZE).map((entry, idx) => {
                              const isSelf = selfEntry && entry.rank === selfEntry.rank;
                              return (
                              <div key={entry.rank} className={`flex items-center justify-between p-4 border-b border-white/5 hover:bg-white/5 transition-colors ${isSelf ? 'bg-white/5 border border-tech-blue/30 rounded' : ''}`}>
                                  <div className="flex items-center gap-4">
                                      ...
                                      <div>
                                          <div className="text-sm font-bold text-white flex items-center gap-2">
                                            {entry.nickname}
                                            {isSelf && <span className="text-[10px] px-2 py-0.5 rounded bg-tech-blue/20 text-tech-blue font-mono">YOU</span>}
                                          </div>
                                          <div className="text-[10px] font-mono text-gray-500">{entry.address}</div>
                                      </div>
                                  </div>
                                  <div className="font-mono text-sm text-white font-bold">{entry.totalEarned.toLocaleString()} <span className="text-[10px] text-gray-500">$mEMO</span></div>
                              </div>);
                          })}
                          <div className="flex justify-center items-center gap-3 pt-2">
                             <button
                      ...
                      <div className="space-y-6">
                          <HudPanel className="p-6">
                              <div className="flex justify-between items-start mb-4">
                                  <div>
                                      ...
                                      {user.invitedBy && <div className="text-xs text-gray-500 mt-1">Invited by {user.invitedBy}</div>}
                                  </div>
                                  {(inviteCodeInfo.locked || inviteCodeInfo.persisted) && (
                                      <div className="px-2 py-1 rounded bg-tech-blue/10 text-tech-blue text-[10px] font-bold">LOCKED</div>
                                  )}
                              </div>
                                  ...
                                  <GameButton 
                                      onClick={() => {
                                          ...
                                          if (!res.ok) {
                                              ...
                                              setInviteMessage(`Bound. Inviter: ${res.invitedBy || 'Community Member'}`);
                                          }
                                      }}
                          ...
                          <HudPanel className="p-6">
                              ...
                              <div className="space-y-3">
                                  ...
                                  <div className="bg-black/40 border border-white/10 rounded px-3 py-2 font-mono text-sm text-white">{inviteCodeDisplay}</div>
                                  <div className="text-sm font-bold text-white flex items-center justify-between">
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] overflow-hidden border border-tech-blue/50 rounded-lg bg-[#0a0d14] shadow-[0_0_30px_rgba(0,243,255,0.25)]">
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[90vh]">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-xs text-tech-blue uppercase tracking-[0.2em]">Submission Details</div>
                  <div className="text-[11px] text-gray-400 font-mono mt-1">ID: {selectedTask.id}</div>
                </div>
                <div className="text-[11px] text-gray-500 font-mono">{new Date(selectedTask.timestamp).toLocaleString()}</div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-[#0c111a] border border-white/10 rounded-md p-5 shadow-inner space-y-5">
                  <div className="flex justify-between items-center pb-3 border-b border-white/10">
                    <h4 className="text-sm font-bold uppercase tracking-wider text-white">Label Summary</h4>
                    <div className="flex items-center gap-2 text-[11px] text-gray-400 font-mono">
                      {selectedTask.status === 'LABELED' && <span className="text-green-500 font-bold flex items-center gap-1"><CheckCircle2 size={12} /> Accepted</span>}
                      {selectedTask.status === 'AUDITING' && <span className="text-yellow-500 font-bold flex items-center gap-1"><Timer size={12} className="animate-spin" /> Verifying</span>}
                      {selectedTask.status === 'REJECTED' && <span className="text-red-500 font-bold flex items-center gap-1"><XCircle size={12} /> Rejected</span>}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-sm text-white">
                    ...
                    <div>
                      ...
                      <div className="text-[13px] font-bold">
                        {selectedTask.status === 'LABELED' && <span className="text-green-500">Accepted</span>}
                        {selectedTask.status === 'AUDITING' && <span className="text-yellow-500">Verifying</span>}
                        {selectedTask.status === 'REJECTED' && <span className="text-red-500">Rejected</span>}
                      </div>
                      {selectedTask.failReason && (
                        <div className="text-[11px] text-gray-400 mt-1">Reason: {selectedTask.failReason}</div>
                      )}
                    </div>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-y-2 text-xs text-gray-300">
                      <div>Face Clear: <span className="text-white font-bold">{selectedTask.draftData?.isClear === true ? 'Yes' : selectedTask.draftData?.isClear === false ? 'No' : 'N/A'}</span></div>
                      <div>Emotion Intensity: <span className="text-white font-bold">{selectedTask.draftData?.intensity ?? 'N/A'}</span></div>
                      <div>Natural or Pose: <span className="text-white font-bold">{selectedTask.draftData?.isStaged === true ? 'Pose' : selectedTask.draftData?.isStaged === false ? 'Natural' : 'N/A'}</span></div>
                      <div>Continuity: <span className="text-white font-bold">{selectedTask.draftData?.arousal ?? 'N/A'}</span></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full bg-[#0c111a] border border-white/10 p-4 rounded-md shadow-inner flex flex-col gap-3">
                  ...
                  <div className={`flex-1 border border-dashed border-white/15 bg-black/40 rounded-md flex items-center justify-center min-h-[280px] text-gray-500 text-xs overflow-hidden relative ${selectedTask.imageUrl ? 'cursor-pointer' : ''}`}
                      ...
                      <>
                        ...
                        {!photoRevealed && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-white text-[11px] bg-black/30">
                            <div className="px-3 py-1 bg-black/60 rounded-full border border-white/15">Blurred for privacy</div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-[#0a0a0f] border border-white/10 rounded-xl w-full max-w-md p-6 relative">
             ...
             <div className="flex justify-end gap-3 mt-4">
                <GameButton variant="ghost" onClick={() => setNicknameModalOpen(false)} className="px-4 py-2 text-[11px]">
                  Cancel
                </GameButton>
                <GameButton
                  onClick={() => {
