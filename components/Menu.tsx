...
const links = [
  ...
  { id: 'support', title: 'Support', desc: '24/7 Help Center', color: 'text-blue-400', icon: LifeBuoy },
];
      ...
      {links.map((link) => (
        ...
        >
          <div className="flex justify-between items-start w-full">
            <div className={`w-8 h-8 rounded-full bg-white/5 flex items-center justify-center ${link.color} group-hover:bg-white/10 transition-colors`}>
               <link.icon size={16} />
            </div>
            <ArrowUpRight className="w-4 h-4 text-gray-600 group-hover:text-white transition-colors" />
          </div>
          <div>
            <h3 className="font-bold text-lg text-white group-hover:translate-x-1 transition-transform">{link.title}</h3>
